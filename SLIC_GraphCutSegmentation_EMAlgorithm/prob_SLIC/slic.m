...
for iter=1:10
    ...
    for j = 1:size(Xnew,2)
        for i = 1:size(Xnew,1)
           ...
           %checking if the calculated distance is lesser than 
...
%     if(iter==1)
%        figure(1);
%        imagesc(log(Dorig)); title('Error Map at initialization');
%     end
end
% figure(2);
...
for j=1:size(Xnew,2)
    for i=1:size(Xnew,1)
        ...
        if sum(mask(:))<0.3*S^2
            ...
            %finding the distance between the pixels in a samall region
            %around pixel and cluster centre
            ...
            %labelling all pixels at least distance from the new cluster
            %centre to that cluster value
