...
%%% computing Q matrix
    ...
    for k=1:100
        ...
        [frames,~]=find(~isnan(Xorig(:,a(k)))); %Finding those frames that had isNan corresponding to a point
        ...
        tn2=size(ptsnew)
        Snew=Anew\ptsnew;
        Dneeded(:,k)=A*Snew; %Dneeded computed by multiplying original affine matrix and new shape vector.
    end
